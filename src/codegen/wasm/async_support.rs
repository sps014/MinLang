...
impl<'a> WasmGenerator<'a> {
    ...
    fn async_slots(
        ...
        let mut entries: Vec<(String, String)> = locals
            ...
            .map(|(name, ty)| {
                (
                    name,
                    WasmGenerator::get_wasm_type_from(self.resolve_type(&ty.get_type()))
        ...
        for stmt in function.body.iter() {
            match stmt {
                StatementNode::Declaration(tok, _, ExpressionNode::Await(child), _) => {
                    let next = segs.len() + 1;
                    segs.push(Segment {
                        resume: std::mem::replace(&mut resume, Resume::BindLocal(tok.text.clone())),
                        plain: std::mem::take(&mut plain),
                        end: SegEnd::Suspend(child, next),
                    });
                }
                StatementNode::AwaitStmt(child) => {
                    let next = segs.len() + 1;
                    segs.push(Segment {
                        resume: std::mem::replace(&mut resume, Resume::Discard),
                        plain: std::mem::take(&mut plain),
                        end: SegEnd::Suspend(child, next),
                    });
                }
                StatementNode::Return(Some(ExpressionNode::Await(child))) => {
                    let next = segs.len() + 1;
                    segs.push(Segment {
                        resume: std::mem::replace(&mut resume, Resume::ReturnAwaited),
                        plain: std::mem::take(&mut plain),
                        end: SegEnd::Suspend(child, next),
                    });
                }
    ...
    pub fn build_async_intrinsic_call(
        &mut self,
        name: &str,
        ...
        match name {
            ...
            intrinsics::PROMISE_ALL | intrinsics::PROMISE_ANY | intrinsics::PROMISE_RACE => {
                ...
                let runtime = if name == intrinsics::PROMISE_ALL {
                    "$dream_all"
                } else {
                    "$dream_any"
                };
