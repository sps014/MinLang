...
impl Parser {
    ...
    pub fn parse(&mut self) -> SyntaxTree {
        let expresion = self.parse_while();
    ...
    fn parse_while(&mut self) -> Box<SyntaxNode> {
        if self.get_current().kind == SyntaxKind::KeyWordToken && self.get_current().text == "while"
        {
            let whi = self.next_token();
    ...
    fn parse_assignment(&mut self) -> Box<SyntaxNode> {
        if self.peek(0).kind == SyntaxKind::IdentifierToken
            && self.peek(1).kind == SyntaxKind::EqualToken
        {
    ...
    fn parse_primary_expression(&mut self) -> Box<SyntaxNode> {
        if self.get_current().kind == SyntaxKind::OpenParenthesisToken {
            ...
            return Box::new(SyntaxNode::ParenthesizedExpressionSyntax(
