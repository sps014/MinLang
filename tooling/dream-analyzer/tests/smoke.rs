...
fn completions_work_on_full_sample() {
    ...
    assert!(len > 0, "expected completions on the playground sample, got: {}", json);
    assert!(json.contains("\"label\":\"add\""), "expected `add`, got: {}", json);
    assert!(json.contains("\"label\":\"origin\""), "expected local `origin`, got: {}", json);
}
...
fn member_completions_after_dot() {
    ...
    assert!(json.contains("\"label\":\"x\""), "expected field `x` after dot, got: {}", json);
    // Strict member access: enums must NOT leak into member completions.
    assert!(!json.contains("\"kind\":\"enum\""), "enums should not appear after `.`, got: {}", json);
}
...
fn this_member_completions_resolve_to_owner() {
    ...
    assert!(json.contains("\"label\":\"x\""), "expected `this` to resolve to Point fields, got: {}", json);
    assert!(json.contains("\"label\":\"y\""), "expected field `y`, got: {}", json);
}
...
fn unknown_receiver_yields_no_members() {
    ...
    let (line, col) = line_char(src, dot);
    let json = completions(src, line, col);
    assert_eq!(json, "[]", "unknown receiver should yield no members, got: {}", json);
}
